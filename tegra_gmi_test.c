...
static inline unsigned long snor_tegra_readl(struct tegra_gmi_char_info *tsnor, unsigned long reg) {
	return readl(tsnor->base + reg);
}

static inline void snor_tegra_writel(struct tegra_gmi_char_info *tsnor, unsigned long val, unsigned long reg) {
...
static int gmichar_chrdev_open(struct inode *inode, struct file *file) {
	...
	if(test_and_set_bit(0, &info->gmi_chrdev_is_open)) return -EBUSY;
	return 0;
}
...
static ssize_t gmichar_chrdev_read_helper(char *dest, int *src, size_t size) {
	int i;
	...
	udelay(1);
	for(i = 0; i < size; i++) dest[i] = __raw_readb(src + i);
	udelay(1);
	return size;
}

static loff_t gmichar_chrdev_llseek(struct file *filp, loff_t offset, int origin) {
	ssize_t ret;
	struct tegra_nor_chip_parms *chip_parm = info->plat;
	struct cs_info *csinfo = &chip_parm->csinfo;
	...
	if(offset < 0 || offset > csinfo->size) offset = -EINVAL;
	else filp->f_pos = offset;
	
	return offset;
}

static ssize_t gmichar_chrdev_read(struct file *filp, char *data, size_t size, loff_t *loff) {
	ssize_t ret;
	char *vbuf = NULL;
	...
	struct cs_info *csinfo = &chip_parm->csinfo;
	if(info == NULL) return -EINVAL;
	if(*loff + size > csinfo->size) return -EINVAL;
	vbuf = vmalloc(size);
	if(vbuf == NULL) return -EFAULT;
	ptr = csinfo->virt;
	ptr = ptr + (*loff);
	gmi_lock(info);
	ret = gmichar_chrdev_read_helper(vbuf, ptr, size);
	if(copy_to_user(data, vbuf, size)) return -EFAULT;
	*loff = *loff + ret;
	vfree(vbuf);
	gmi_unlock(info);
	return ret;
...
static ssize_t gmichar_chrdev_write_helper(unsigned int *dest, char *src, size_t size) {
	int i;
	snor_tegra_writel(info, info->init_config, TEGRA_SNOR_CONFIG_REG);
	snor_tegra_writel(info, info->timing1_read, TEGRA_SNOR_TIMING1_REG);
	snor_tegra_writel(info, info->timing0_read, TEGRA_SNOR_TIMING0_REG);
	udelay(1);
	for (i = 0; i < size; i++) __raw_writeb(src[i], dest + i);
	udelay(1);
	return size;
}

static ssize_t gmichar_chrdev_write(struct file *filp, const char *data, size_t size, loff_t *loff) {
	ssize_t ret;
	struct tegra_nor_chip_parms *chip_parm = info->plat;
	struct cs_info *csinfo = &chip_parm->csinfo;
	char *vbuf = NULL;
	...
	if(info == NULL) return -EINVAL;
	vbuf = vmalloc(size);
	if(copy_from_user(vbuf, data, size)) return -EFAULT;
	ptr = csinfo->virt;
	ptr = ptr + *loff;
	gmi_lock(info);
...
static int __devinit tegra_gmi_char_probe(struct platform_device *pdev) {
	...
	info = devm_kzalloc(dev, sizeof(struct tegra_gmi_char_info), GFP_KERNEL);
	if(!info) return -ENOMEM;
	info->base = ((void __iomem *)IO_APB_VIRT + (TEGRA_SNOR_BASE - IO_APB_PHYS));
	info->plat = plat;
	info->dev = dev;
	#ifdef CONFIG_TEGRA_GMI_ACCESS_CONTROL
...
static int __devexit tegra_gmi_char_remove(struct platform_device *pdev) {
	if(info->gmi_class) class_destroy(info->gmi_class);
